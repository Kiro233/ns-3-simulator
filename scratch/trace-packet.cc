...
struct UeConfigParams {
    Ptr<NrHelper> nrHelper;               // 指向NrHelper的指针，用于NR操作
    NodeContainer& ueLowLatContainer;     // 引用用户设备的容器，专用于低延迟业务
    NetDeviceContainer& ueLowLatNetDev;   // 引用用户设备的网络设备容器
    Ipv4InterfaceContainer& ueLowLatIpIface;  // 引用IPv4接口容器
    Ptr<Node> remoteHost;                 // 指向远程主机的指针
    EpsBearer& lowLatBearer;              // 引用电子数据保护承载结构
    Ptr<EpcTft> lowLatTft;                // 指向EPC流量流模板的指针
    ApplicationContainer& clientApps;     // 引用应用容器，用于存储客户端应用
    Time simTime;                         // 模拟时间
    UdpClientHelper& dlClientLowLat;       // UDP客户端助手
};

// 函数定义：配置下一个用户设备节点
void ConfigureNextUeNode(UeConfigParams params) {
    if (ueNum < params.ueLowLatContainer.GetN()) {  // 检查是否还有未配置的用户设备
        
    }
}

static void
PacketTrace(const Ptr< const Packet > packet, const Address &srcAddress, const Address &destAddress)
{
    NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "\t" << srcAddress << "\t" << destAddress);
}

static void
PacketTrace0(const Ptr< const Packet > packet, const Address &srcAddress, const Address &destAddress)
{
    NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "\t" << destAddress);
}
...
int main(int argc, char* argv[]) {
    ...
    bool logging = true;  // 是否启用日志
    bool doubleOperationalBand = false;  // 是否使用双操作频段

    uint32_t udpPacketSizeULL = 512;  // 超低延迟业务的UDP包大小
    uint32_t lambdaULL = 60;  // 超低延迟业务的包发送率

    // Time simTime = MilliSeconds(1000);  // 模拟时间
    Time simTime = Seconds(1);  // 模拟时间设置
    Time udpAppStartTime = MilliSeconds(400);  // UDP应用启动时间

    uint16_t numerologyBwp1 = 4;  // 带宽部分1的数字化
    double centralFrequencyBand1 = 28e9;  // 频带1的中心频率
    double bandwidthBand1 = 50e6;  // 频带1的带宽
    ...
    cmd.AddValue("gNbNum", "Number of gNbs in topology", gNbNum);
    ...
    cmd.AddValue("packetSizeUll", "Packet size for ultra low latency traffic", udpPacketSizeULL);
    ...
    if (logging) {  // 如果启用日志，则激活相关组件的日志功能
        ...
        LogComponentEnable("LtePdcp", LOG_LEVEL_ERROR);
    ...
    gridScenario.SetHorizontalBsDistance(10.0);  // 设置基站水平距离
    gridScenario.SetVerticalBsDistance(10.0);  // 设置基站垂直距离
    gridScenario.SetBsHeight(10);  // 设置基站高度
    ...
    gridScenario.SetScenarioHeight(3);  // 设置场景高度
    gridScenario.SetScenarioLength(3);  // 设置场景长度
    ...
    NS_LOG_INFO("Creating " << gridScenario.GetUserTerminals().GetN() << " user terminals and "
                            << gridScenario.GetBaseStations().GetN() << " gNBs");  // 记录日志信息
    ...
    p2ph.SetChannelAttribute("Delay", TimeValue(Seconds(0.010)));  // 设置通道延迟
    ...
    UdpClientHelper dlClientLowLat[MaxUE];  // 创建UDP客户端助手
    ApplicationContainer clientApps[MaxUE];  // 创建客户端应用容器
    ...
    for (uint32_t j = 0; j < gridScenario.GetUserTerminals().GetN(); j++)
    {
        ...
        dlClientLowLat[j].SetAttribute("PacketSize", UintegerValue(udpPacketSizeULL));  // 设置包大小
        dlClientLowLat[j].SetAttribute("Interval", TimeValue(Seconds(1.0 / lambdaULL)));  // 设置发送间隔

        EpsBearer lowLatBearer(EpsBearer::NGBR_LOW_LAT_EMBB);  // 创建EPS承载配置，指定为低延迟非保证比特率
        Ptr<EpcTft> lowLatTft = Create<EpcTft>();  // 创建流量流模板
        EpcTft::PacketFilter dlpfLowLat;  // 创建数据包过滤器
        dlpfLowLat.localPortStart = dlPortLowLat;
        dlpfLowLat.localPortEnd = dlPortLowLat;
        lowLatTft->Add(dlpfLowLat);  // 将过滤器添加到流量流模板
        ...
        clientApps[j].Start(udpAppStartTime + MilliSeconds(100*j));
        clientApps[j].Stop(simTime);

        serverApps.Get(j)->TraceConnectWithoutContext("RxWithAddresses",
                                                        MakeCallback(&PacketTrace));
        // serverApps.Get(0)->TraceConnectWithoutContext("RxWithAddresses",
        //                                                 MakeCallback(&PacketTrace)); 
        // serverApps.Get(1)->TraceConnectWithoutContext("RxWithAddresses",
        //                                                 MakeCallback(&PacketTrace));                                                                                     
    }
    ...
    //     nrHelper,e
    //     std::ref(ueLowLatContainer),
    //     std::ref(ueLowLatNetDev),
    //     std::ref(ueLowLatIpIface),
    //     remoteHost,
    //     std::ref(lowLatBearer),
    //     lowLatTft,
    //     std::ref(clientApps),
    //     simTime,
    //     dlClientLowLat,
    ...
    // Simulator::Schedule(MilliSeconds(200), &ConfigureNextUeNode, params);
    Simulator::Stop(simTime);  // 设置模拟停止时间
    Simulator::Run();  // 运行模拟

    // monitor->CheckForLostPackets();  // 检查丢失的包
    ...
    // double averageFlowThroughput = 0.0;  // 平均流吞吐量
    // double averageFlowDelay = 0.0;  // 平均流延迟

    // std::ofstream outFile;  // 创建输出文件流
    // std::string filename = outputDir + "/" + simTag;  // 设置文件名
    // outFile.open(filename.c_str(), std::ofstream::out | std::ofstream::trunc);  // 打开文件
    // if (!outFile.is_open()) {
    //     std::cerr << "Can't open file " << filename << std::endl;
    //     return 1;
    // }

    // outFile.setf(std::ios_base::fixed);  // 设置输出格式为固定点表示法

    // double flowDuration = (simTime - udpAppStartTime).GetSeconds();  // 计算流持续时间
    // for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i) {  // 遍历所有流统计数据
    ...
    //         averageFlowThroughput += i->second.rxBytes * 8.0 / flowDuration / 1000 / 1000;  // 累加平均吞吐量
    //         averageFlowDelay += 1000 * i->second.delaySum.GetSeconds() / i->second.rxPackets;  // 累加平均延迟
    ...
    // double meanFlowThroughput = averageFlowThroughput / stats.size();  // 计算平均流吞吐量
    // double meanFlowDelay = averageFlowDelay / stats.size();  // 计算平均流延迟
    ...
    // std::ifstream f(filename.c_str());  // 打开文件用于读取

    // if (f.is_open()) {
    //     std::cout << f.rdbuf();  // 输出文件内容
    // }
