...
bool IsAvaiableGear(double bandWidth, int newGear) {
    double totalTime = 0.0;
    //UE 计算能力3TFLOPS 
    totalTime += gearTable.ueComputationLoad[newGear]*1000.0/3;
    //cloud 计算能力14TFLOPS
    totalTime += gearTable.cloudComputationLoad[newGear]*1000.0/14;
    //transmission RTT
    totalTime += ((double)gearTable.PacketSizeRequirement[newGear])*8/1000/1000/bandWidth * 2.0;

    if (totalTime < 50) return 1;
    else return 0;
}
    ...
    for (int i = 0; i < params.ueAmount; i++) {
        ...
        if (recordDelay[params.windows_size-1] == Seconds(0)) {
            ...
            if (rttDelay > 50 && !params.clientApps[ueNum]->is_lock()) {
                ...
                if (Simulator::Now().GetSeconds() - params.clientApps[ueNum]->GetLastGearChangeTime().GetSeconds() >= 15 && ueGearList[ueNum] > 0) {
                    int newGear = ueGearList[ueNum]-1;
                    if (IsAvaiableGear(FlowThroughput, newGear)) {
                        NS_LOG_UNCOND("  time " << Simulator::Now().GetSeconds() << " ue " << ueNum << " will drop gear to " << newGear << " at " << Simulator::Now().GetMilliSeconds() + params.adjustDelay.GetMilliSeconds() << "ms");
                        params.clientApps[ueNum]->lock();
                        Simulator::Schedule(params.adjustDelay, &changeGear, ueNum, params.clientApps, newGear);
                    }
                }
