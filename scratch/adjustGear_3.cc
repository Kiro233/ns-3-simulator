/*
NS_LOG="UdpServer=info|prefix_time|prefix_node|prefix_func" ./ns3 run scratch/adjustGear_3 > scratch/log.adjustGear_3.server.out 2>&1
*/
...
uint16_t ueNumPergNb = 2;         // 每个gNb的用户设备数量
uint16_t dlPortLowLat = 1234;  // 设置低延迟业务的目的端口
uint32_t lambdaULL = 60;  // 超低延迟业务的包发送率
Time adjustInteval = MilliSeconds(100); //调档间隔
// Time adjustInteval = MicroSeconds(1001); //调档间隔
// uint32_t StreamNum = -(ueNumPergNb * gNbNum -1);
uint32_t StreamNum = 1;
...
struct GearTable {
    ...
    double bandwidthRequirement[GearNum];   // 带宽 Mbps
    double PacketSizeRequirement[GearNum];  // 包大小
};
...
// void ChangePacketSize(UdpClientHelper dlClientLowLat, uint32_t size, ApplicationContainer clientApp, Ptr<Node> remoteHost)
// {
//   dlClientLowLat.SetAttribute("PacketSize", UintegerValue(size));
//   clientApp.Add(dlClientLowLat.Install(remoteHost));
// }
...
int relatSchedule(int ueID, double bandWidth) {
    ...
    for(int l=1;l<GearNum;l++) {
        ...
        //transmission RTT
        totalTime += gearTable.bandwidthRequirement[l]*2.0/bandWidth;
        if(totalTime > 50) {
            newGear = -1;
            continue;
        }
...
void estateFlows(Ptr<ns3::FlowMonitor>& monitor, Ptr<Ipv4FlowClassifier>& classifier) {
    FlowMonitor::FlowStatsContainer stats = monitor->GetFlowStats();  // 获取流统计数据
    for (uint32_t j = StreamNum; j <= StreamNum + gNbNum*ueNumPergNb - 1; j++) {
        std::map<FlowId, FlowMonitor::FlowStats>::iterator i = stats.find(j);
        
        Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow(i->first);  // 获取流的五元组信息
        std::stringstream protoStream;  // 创建协议流
        protoStream << (uint16_t)t.protocol;  // 将协议编号转换为字符串
        if (t.protocol == 6) {  // TCP协议
            protoStream.str("TCP");
        }
        if (t.protocol == 17) {  // UDP协议
            protoStream.str("UDP");
        }

        if (i->second.rxPackets > 0) {  // 如果有接收包
            double Delay = 0.0;
            double FlowThroughput = 0.0;
            double rxDuration = i->second.delaySum.GetSeconds();
            FlowThroughput = i->second.rxBytes * 8.0 / rxDuration / 1000 / 1000;  // 吞吐量
            Delay = 1000 * i->second.delaySum.GetSeconds() / i->second.rxPackets;

            int modValue = gNbNum * ueNumPergNb;
            int flowNum = (i->first%modValue == 0) ? modValue : i->first%modValue;

            NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "Flow " << flowNum << " (" << t.sourceAddress << ":" << t.sourcePort << " -> "
                    << t.destinationAddress << ":" << t.destinationPort << ") proto "
                    << protoStream.str() << "\n");

            NS_LOG_UNCOND("  rxDuration: " << rxDuration << "\n");

            NS_LOG_UNCOND("  Tx Packets: " << i->second.txPackets << "\n");
            NS_LOG_UNCOND("  Tx Bytes:   " << i->second.txBytes << "\n");
            NS_LOG_UNCOND("  TxOffered:  " << i->second.txBytes * 8.0 / rxDuration / 1000.0 / 1000.0
                    << " Mbps\n");
            NS_LOG_UNCOND("  Rx Packets: " << i->second.rxPackets);
            NS_LOG_UNCOND("  Rx Bytes:   " << i->second.rxBytes << "\n");

            NS_LOG_UNCOND("  Lost Packets: " << i->second.lostPackets << "\n");
            NS_LOG_UNCOND("  Throughput: " << FlowThroughput << " Mbps");
            NS_LOG_UNCOND("  RTT delay: " << 1000 * i->second.delaySum.GetSeconds() / i->second.rxPackets * 2 << " ms");
            NS_LOG_UNCOND("  Mean jitter: " << 1000 * i->second.jitterSum.GetSeconds() / i->second.rxPackets << " ms");
            
            NS_LOG_UNCOND("  RTT Delay: " << Delay * 2 << " ms");

        } else {
            // NS_LOG_UNCOND("  Throughput:  0 Mbps");
            // NS_LOG_UNCOND("  Mean delay:  0 ms");
            // NS_LOG_UNCOND("  Mean jitter: 0 ms");
        }
...
//
void adjustGear(Ptr<ns3::FlowMonitor>& monitor, Ptr<Ipv4FlowClassifier>& classifier) {
    FlowMonitor::FlowStatsContainer stats = monitor->GetFlowStats();  // 获取流统计数据
...
// 函数定义：动态调档
void startApps(UeConfigParams params) {
    NS_LOG_UNCOND("  time " << Simulator::Now().GetSeconds() << "\n");

    for (uint32_t j = 0; j < params.gridScenario.GetUserTerminals().GetN(); j++)
    {
        params.clientApps[j].StopApplication();
        
    }
    
    // adjustGear(params.monitor, params.classifier);

    // UdpClientHelper dlClientLowLat[ueNumPergNb * gNbNum];  // 创建UDP客户端助手
    // UdpClient clientApps[ueNumPergNb * gNbNum];  // 创建客户端应用容器
    ...
    //     dlClientLowLat[j].SetAttribute("Interval", TimeValue(Seconds(1.0 / lambdaULL)));  // 设置发送间隔

    //     // 配置并安装客户端应用到远程主机
    //     Ptr<Node> ue = params.ueLowLatContainer.Get(j);
    //     Ptr<NetDevice> ueDevice = params.ueLowLatNetDev.Get(j);
    //     Address ueAddress = params.ueLowLatIpIface.GetAddress(j);
    //     dlClientLowLat[j].SetAttribute("RemoteAddress", AddressValue(ueAddress));
    //     clientApps[j] = *dlClientLowLat[j].Install(params.remoteHost);

    //     clientApps[j].SetStartTime(MilliSeconds(0));
    //     clientApps[j].SetStopTime(adjustInteval);
    // }
    // StreamNum += gNbNum*ueNumPergNb;

    // Simulator::Schedule(adjustInteval, &startApps, params);
}
...
int main(int argc, char* argv[]) {
    ...
    Time simTime = Seconds(1);  // 模拟时间设置
    Time udpAppStartTime = MilliSeconds(0);  // UDP应用启动时间
    ...
    cmd.AddValue("lambdaUll", "UDP packets per second for ultra low latency traffic", lambdaULL);
    ...
    gridScenario.SetRows(3);  // 设置行数
    gridScenario.SetColumns(7);  // 设置列数
    gridScenario.SetHorizontalBsDistance(5.0);  // 设置基站水平距离
    gridScenario.SetVerticalBsDistance(5.0);  // 设置基站垂直距离
    gridScenario.SetBsHeight(100);  // 设置基站高度
    gridScenario.SetUtHeight(1.5);  // 设置用户设备高度
    gridScenario.SetSectorization(GridScenarioHelper::SINGLE);  // 设置单一扇区化
    gridScenario.SetBsNumber(gNbNum);  // 设置基站数量
    gridScenario.SetUtNumber(ueNumPergNb * gNbNum);  // 设置用户设备总数
    gridScenario.SetScenarioHeight(200);  // 设置场景高度
    gridScenario.SetScenarioLength(200);  // 设置场景长度
    randomStream += gridScenario.AssignStreams(randomStream);  // 分配随机数流
    gridScenario.CreateScenario();  // 创建场景

    NS_LOG_UNCOND("  Scenario create successe " << Simulator::Now().GetSeconds() << "\n");

    NodeContainer ueLowLatContainer;  // 创建低延迟用户设备容器
    // NodeContainer ueVoiceContainer;  // 创建语音用户设备容器
    ...
    nrHelper->SetGnbAntennaAttribute("NumColumns", UintegerValue(8));  // 设置gNb天线列数
    ...
    nrHelper->SetGnbBwpManagerAlgorithmAttribute("NGBR_LOW_LAT_EMBB", UintegerValue(bwpIdForLowLat));  // 设置gNb带宽部分管理算法属性
    nrHelper->SetUeBwpManagerAlgorithmAttribute("NGBR_LOW_LAT_EMBB", UintegerValue(bwpIdForLowLat));  // 设置用户设备带宽部分管理算法属性
    ...
    p2ph.SetChannelAttribute("Delay", TimeValue(MilliSeconds(10)));  // 设置通道延迟
    ...
    serverApps.Add(dlPacketSinkLowLat.Install(ueLowLatContainer));  // 安装UDP服务器到低延迟用户设备

    // UdpClientHelper dlClientLowLat[ueNumPergNb * gNbNum];  // 创建UDP客户端助手
    // ApplicationContainer clientApps[ueNumPergNb * gNbNum];  // 创建客户端应用容器
    ...
    //     dlClientLowLat[j].SetAttribute("RemoteAddress", AddressValue(ueAddress));
    //     clientApps[j].Add(dlClientLowLat[j].Install(remoteHost));
    //     clientApps[j].Start(MilliSeconds(400));
    //     // clientApps[j].Stop(MilliSeconds(400) + adjustInteval);
    //     clientApps[j].Stop(simTime);
    // }
    ...
    for (uint32_t j = 0; j < gridScenario.GetUserTerminals().GetN(); ++j)
    {
        ...
        clientApps[j].SetStartTime(MilliSeconds(400));
    ...
    UeConfigParams params{  // 创建用户设备配置参数结构
        classifier,
        monitor,
        gridScenario,
        ueLowLatContainer,
    ...
    // Simulator::Schedule(MilliSeconds(400) + adjustInteval, &startApps, params);
    // Simulator::Schedule(simTime, &estateFlows, monitor, classifier);

    Simulator::Schedule(MilliSeconds(500), &startApps, params);
    // Simulator::Schedule(MicroSeconds(1001), &adjustGear);
