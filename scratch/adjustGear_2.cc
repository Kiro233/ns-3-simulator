...
/*
NS_LOG="UdpServer=info|prefix_time|prefix_node|prefix_func" ./ns3 run scratch/adjustGear_2 > scratch/log.adjustGear_2.server.out --command-template="%s --gNbNum=3 --ueNumPergNb=5 --adjustInteval=0.005 --adjustDelay=0.05" 2>&1
*/
// 9-9
...
//             for (int j = WINDOWS_SIZE-1; j >= 0; j--) {
//                 if (i->second.recordDelay[j] != Seconds(0)) {
//                     recordNum++;
//                     recordDelaySum += i->second.recordDelay[j];
//                 }
//             }

//             meanTransDelay = recordDelaySum.GetMilliSeconds() / recordNum;  // 传输时延
//             FlowThroughput = i->second.lastPKSize * 8 / meanTransDelay / 1000 / 1000;    // 吞吐量Mbps
...
//             // NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "ue " << i->first - 1 << " (" << t.sourceAddress << ":" << t.sourcePort << " -> "
//             //         << t.destinationAddress << ":" << t.destinationPort << ") proto "
//             //         << protoStream.str() << "\n");

//             // NS_LOG_UNCOND("  gear: " << gear << "\n");       

//             // NS_LOG_UNCOND("  Tx Packets: " << i->second.txPackets << "\n");
//             // NS_LOG_UNCOND("  Tx Bytes:   " << i->second.txBytes << "\n");
//             // NS_LOG_UNCOND("  TxOffered:  " << i->second.txBytes * 8.0 / recordDelaySum.GetSeconds() / 1000.0 / 1000.0
//             //         << " Mbps\n");
//             // NS_LOG_UNCOND("  Rx Packets: " << i->second.rxPackets);
//             // NS_LOG_UNCOND("  Rx Bytes:   " << i->second.rxBytes << "\n");

//             // NS_LOG_UNCOND("  Lost Packets: " << i->second.lostPackets << "\n");
//             // NS_LOG_UNCOND("  帧正确率: " << (double)i->second.rxPackets / (double)i->second.txPackets << "\n");
...
//             // NS_LOG_UNCOND("  RTT Delay: " << rttDelay << " ms");
...
void adjustGear(AdjustGearParams& params
    ...
    for (int i = 0; i < params.ueAmount; i++) {
        ...
        if (recordDelay[WINDOWS_SIZE-1] == Seconds(0)) {
            continue;
        }
    ...
    //         // use pre WINDOWS_SIZE
    //         if (i->second.recordAllDelay[WINDOWS_SIZE-1] == Seconds(0)) {
    //             continue;
    //         }
    //         else {
    //             for (int j = WINDOWS_SIZE-1; j >= 0; j--) {
    //                 if (i->second.recordAllDelay[j] != Seconds(0)) {
    //                     recordNum++;
    //                     recordDelaySum += i->second.recordAllDelay[j];
    //                 }
    //             }
    ...
    //         NS_LOG_UNCOND("  all last Delay: " << i->second.recordAllGear[0] << "  "
    //          << i->second.recordAllGear[1] << "  "
    //           << i->second.recordAllGear[2] << "  "
    //            << i->second.recordAllGear[3] << "  "
    //             << i->second.recordAllGear[4] << "  "
    //              << i->second.recordAllGear[5] << " ms");


    //         // NS_LOG_UNCOND(Simulator::Now().GetSeconds() << "Flow " << i->first << " (" << t.sourceAddress << ":" << t.sourcePort << " -> "
    //         //         << t.destinationAddress << ":" << t.destinationPort << ") proto "
    //         //         << protoStream.str());

    //         // NS_LOG_UNCOND("  gear: " << gear << "\n");  
    //         // NS_LOG_UNCOND("  recordNum: : " << recordNum);
    //         // NS_LOG_UNCOND("  meanTransDelay: : " << meanTransDelay);
    //         // NS_LOG_UNCOND("  pK size: : " << i->second.lastPKSize);
    //         // NS_LOG_UNCOND("  all last Delay: " << i->second.recordDelay[0] << "  "
    //         //  << i->second.recordDelay[1] << "  "
    //         //   << i->second.recordDelay[2] << "  "
    //         //    << i->second.recordDelay[3] << "  "
    //         //     << i->second.recordDelay[4] << "  "
    //         //      << i->second.recordDelay[5] << " ms");

    //         // NS_LOG_UNCOND("  ue Delay: " << gearTable.ueComputationLoad[gear]*1000.0/3 << " ms");
    //         // NS_LOG_UNCOND("  cloud Delay: " << gearTable.cloudComputationLoad[gear]*1000.0/14 << " ms");
    //         // NS_LOG_UNCOND("  rttDelay now is " << rttDelay << "ms" << "\n");

    //         // if (rttDelay > 50 && !clientApps[ueNum]->is_lock()) {
    //         //     // NS_LOG_UNCOND("  ueNum" << ueNum << "change gear");
    //         //     // NS_LOG_UNCOND("  meanTransDelay now is " << meanTransDelay << "ms");
    ...
    //         //         // NS_LOG_UNCOND("  time " << Simulator::Now().GetSeconds() << " ue " << ueNum << " will change Packet Size to " << new_size << " at " << Simulator::Now().GetMilliSeconds() + adjustDelay.GetMilliSeconds() << "ms");
    //         //         clientApps[ueNum]->lock();
    //         //         Simulator::Schedule(adjustDelay, &changeGear, ueNum, clientApps, newGear);
    //         //     }
    //         //     else {
    //         //         // NS_LOG_UNCOND("System Overload");
    //         //         // Simulator::Stop();
