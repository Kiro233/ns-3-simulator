...
FlowMonitor::GetStatsForFlow(FlowId flowId)
{
    ...
    if (iter == m_flowStats.end())
    {
        ...
        ref.lastPKSize = 0;
        for (int i = 0; i < WINDOWS_SIZE; i++) ref.recordDelay[i] = Seconds(0);
        for (int i = 0; i < 2 * WINDOWS_SIZE; i++) ref.recordAllDelay[i] = Seconds(0);
        for (int i = 0; i < 2 * WINDOWS_SIZE; i++) ref.recordAllGear[i] = 0;
        ref.txBytes = 0;
        ref.rxBytes = 0;
        ref.txPackets = 0;
        ref.rxPackets = 0;
                          ...
                          uint32_t packetSize)
{
    ...
    if (packetSize < stats.lastPKSize) {
        for (int i = 0; i < WINDOWS_SIZE-1; i++) stats.recordDelay[i] = Seconds(0);
        stats.recordDelay[WINDOWS_SIZE-1] = delay;
    }
    else {
        for (int i = 0; i < WINDOWS_SIZE-1; i++) stats.recordDelay[i] = stats.recordDelay[i+1];
        stats.recordDelay[WINDOWS_SIZE-1] = delay;
    }
    stats.lastPKSize = packetSize;

    // recordAllDelay
    for (int i = 0; i < 2*WINDOWS_SIZE-1; i++) stats.recordAllDelay[i] = stats.recordAllDelay[i+1];
    stats.recordAllDelay[2*WINDOWS_SIZE-1] = delay;

    // gear
    // SeqTsSizeHeader seqTs;
    // packet->RemoveHeader(seqTs);
    // uint16_t gear = seqTs.GetSize();
    // for (int i = 0; i < 2*WINDOWS_SIZE-1; i++) stats.recordAllGear[i] = stats.recordAllGear[i+1];
    // stats.recordAllGear[2*WINDOWS_SIZE-1] = gear;
