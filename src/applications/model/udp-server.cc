...
Time*
UdpServer::getRecordDelay() {
    return recordDelay;
}

uint16_t*
UdpServer::getRecordGear() {
    return recordGear;
}
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        ...
        if (packet->GetSize() > 0)
        {
            ...
            if (InetSocketAddress::IsMatchingType(from))
            {
                ...
                NS_LOG_INFO("TraceDelay: RX " << " receive "
                                ...
                                << " Gear: " << seqTs.GetSize());
                // recordDelay
                for (int i = 0; i < 2*WINDOWS_SIZE-1; i++) recordDelay[i] = recordDelay[i+1];
                recordDelay[2*WINDOWS_SIZE-1] = Simulator::Now() - seqTs.GetTs();

                // recordGear
                for (int i = 0; i < 2*WINDOWS_SIZE-1; i++) recordGear[i] = recordGear[i+1];
                recordGear[2*WINDOWS_SIZE-1] = seqTs.GetSize();


                // NS_LOG_UNCOND("  all last gear: " << recordGear[0] << "  "
                //     << recordGear[1] << "  "
                //     << recordGear[2] << "  "
                //     << recordGear[3] << "  "
                //         << recordGear[4] << "  "
